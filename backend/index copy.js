...
app.post('/compose', receiveData, async (request, response) => {
    ...
    writeFile(path.join(__dirname, 'src', 'files', `file-${order}-complete.${type}`), Buffer.concat(parts, size)).then(() => {
...
app.post('/compose', receiveData, async (request, response) => {
    ...
    writeFile(path.join(__dirname, 'src', 'files', `file-${name}-complete.${type}`), Buffer.concat(parts, size)).then(() => {
        //console.log('> Compressing File.');
       // require('./src/compress')(`file-${id}-complete`, type);
